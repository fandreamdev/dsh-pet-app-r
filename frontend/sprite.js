...
class PetSprite {
    ...
    dispose() {
        ...
        if (this.broadcastLoopTimer !== null)
            window.clearTimeout(this.broadcastLoopTimer);
    ...
    sendBounds(px, py) {
        ...
        if (window.petBridge) {
            ...
            window.petBridge.setBounds(this.pos.x - this.margin.l, this.pos.y - this.margin.t, this.size + this.margin.l + this.margin.r, this.winH + this.margin.t + this.margin.b, this.pos.x, // 包围盒左上角（碰撞站场用：窗口坐标 ≠ 包围盒坐标）
            this.pos.y, this.size, this.bottomPad, fly ? fly.vx : 0, fly ? fly.vy : 0);
        }
    }
    ...
    currentCenterX() {
        if (this.customPos)
            return this.customPos.rx * VIEW.w;
        return this.pos.x + this.halfW;
    }
    currentCenterY() {
        if (this.customPos)
            return this.customPos.ry * VIEW.h;
    ...
    playIdle() {
        ...
        const animationWeights = this.weights;
    ...
    handleEnded() {
        ...
        const isEvent = Object.values(events).some((pool) => pool.includes(this.anim));
        if (isEvent) {
            if (animations.idle.length)
                this.playOnce(S.pick(animations.idle, this.anim));
            return;
        }
    ...
    tryMove(preferredName) {
        if (this.moveRef !== null || this.pendingMove || this.throwRef !== null)
            return true;
    ...
    startMoveDrive(el) {
        ...
        const step = () => {
            ...
            if (t <= leadSec)
                ...
                ratioX = startRatio + dir * totalRatio * ((t - leadSec) / travelWindow);
        ...
        const step = () => {
            ...
            if (this.physics && this.physics.petCollision) {
                ...
                for (const pid of Object.keys(this.others)) {
                    ...
                    if (!S.rectsOverlap(myBody, otherBody))
                        continue;
                    const hit = S.collidePet({ x: state.x, y: state.y, vx: state.vx, vy: state.vy, size: this.size }, { x: o.x, y: o.y, vx: o.vx, vy: o.vy, size: o.size });
                    if (hit) {
    ...
    onPointerMove(e) {
        ...
        if (!d.dragging) {
            if (Math.hypot(dx, dy) < S.DRAG_THRESHOLD)
                return;
            d.dragging = true;
    ...
    setInteractive(flag) {
        const next = !!flag;
        if (next === this._interactive)
            return; // 只在状态变化时发 IPC，避免逐帧刷屏
        this._interactive = next;
        window.__dshPetDebug.interactive = next;
        if (window.petBridge)
            window.petBridge.setInteractive(next);
    }
    onMouseMove(e) {
    ...
    onClick() {
        const d = this.dragState;
        if (d.active || d.dragging || this.justDragged)
            return;
        ...
        this.stopMove();
        if (!this.animations.clicks.length)
            return;
    ...
    onContextMenu(e) {
        const d = this.dragState;
        if (d.active || d.dragging || this.justDragged || this.menuOpen)
            return;
        e.preventDefault();
    ...
    onMenuAction(leaf) {
        ...
        if (this.animations.moves.actions.some((a) => a.name === leaf.anim)) {
            if (this.tryMove(leaf.anim) === false)
    ...
    renderBubble() {
        ...
        if (hasTier) {
            ...
            for (const r of rows) {
                ...
                if (r.role === 'tier')
                    span.className = 'pet-bub-tier pet-bub-tier-' + r.tier;
            ...
            for (const r of rows) {
                ...
                if (r.role === 'error')
                    div.className = 'pet-bub-err';
                else if (r.role === 'sub')
                    div.className = 'pet-bub-row pet-bub-sub';
                else
                    div.className = 'pet-bub-row';
