...
function reportTele(extra) {
    try {
        if (window.__reportTele)
            window.__reportTele(Object.assign({ t: Date.now() }, extra));
    }
    catch {
        /* ignore */
    }
...
async function boot() {
    try {
        ...
        if (window.petBridge && window.petBridge.startEvents)
            window.petBridge.startEvents(pet.id);
        for (const s of sprites)
            s.playIdle();
